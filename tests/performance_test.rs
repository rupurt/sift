...
fn test_query_embedding_cache_avoids_redundant_calls() {
    ...
    std::fs::write(temp_corpus.path().join("test.txt"), "performance optimization").expect("write test file");

    let call_count = Arc::new(std::sync::atomic::AtomicUsize::new(0));
    let embedder = Arc::new(MockEmbedder { call_count: call_count.clone() });
    ...
    let _response1 = run_search(&request, None, &LocalFileCorpusRepository, Some(embedder.clone())).expect("first search");
    ...
    let _response2 = run_search(&request, None, &LocalFileCorpusRepository, Some(embedder.clone())).expect("second search");
