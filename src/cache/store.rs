...
impl Manifest {
    ...
    pub fn save(&self, path: &Path) -> Result<()> {
        ...
        file.lock_exclusive().context("failed to acquire exclusive lock on manifest")?;
        
        bincode::serialize_into(&file, self)
            .with_context(|| format!("failed to serialize manifest to {}", path.display()))?;
            
        file.unlock().context("failed to release exclusive lock on manifest")?;
        Ok(())
    }
...
pub fn save_blob(blobs_dir: &Path, hash: &str, document: &Document) -> Result<()> {
    ...
    let mut file = File::create(&temp_path).with_context(|| format!("failed to create temp blob {}", temp_path.display()))?;
    
    bincode::serialize_into(&mut file, document)
        .with_context(|| format!("failed to serialize document blob to {}", temp_path.display()))?;
