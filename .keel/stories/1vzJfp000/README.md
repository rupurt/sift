...
- [ ] [SRS-04/AC-01] `sift search <query> <path>` recursively scans supported ASCII/UTF-8 files and returns ranked BM25 results without requiring a prebuilt or persisted index. <!-- verify: cargo test search::bm25 + cargo run -- search "retrieval architecture" .cache/eval/scifact-files --engine bm25, SRS-04:start:end, proof: ac-1.log -->
