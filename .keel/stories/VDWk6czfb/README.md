...
updated_at: 2026-03-10T20:50:54
scope: VDWjylxX2/VDWk0yH65
index: 1
started_at: 2026-03-10T20:50:54
---
...
- [ ] [SRS-01/AC-01] A normal local build reports `sift <semver>-dev (<sha>)`, using a seven-character SHA in the clap `--version` output. <!-- verify: sh -lc 'cd "$(git rev-parse --show-toplevel)" && cargo run -- --version | rg "^sift [0-9]+\\.[0-9]+\\.[0-9]+-dev \\([0-9a-f]{7}\\)$"', SRS-01:start:end, proof: ac-1.log-->
...
- [ ] [SRS-04/AC-04] Shared version-formatting logic is covered by automated tests for release, dev, and `unknown` fallback cases. <!-- verify: cargo test versioning::, SRS-04:start:end, proof: ac-4.log-->
